...
from models.ComCo import ComCo
...
if __name__ == "__main__":
    ...
    noisy_type = '-' # drop_edge drop_node, noisy_node'
    # get arguments

    args = get_link_prediction_args(is_evaluation=False)

    for args.dataset_name in ['wikipedia', 'reddit']:
        ...
        all_node_idx = np.unique(np.concatenate([train_data.src_node_ids, train_data.dst_node_ids], axis=0))
        ...
        for run in range(args.num_runs):
            ...
            if args.model_name == 'TGAT':
                ...
                dynamic_backbone = ComCo(args.top_k, args.top_n, args.alpha, args.beta, args.ratio, train_node_ids=all_node_idx,node_raw_features=node_raw_features, edge_raw_features=edge_raw_features, neighbor_sampler=train_neighbor_sampler,
                                             time_feat_dim=args.time_feat_dim, channel_embedding_dim=args.channel_embedding_dim, patch_size=args.patch_size,
                                             num_layers=args.num_layers, num_heads=args.num_heads, dropout=args.dropout,
                                             max_input_sequence_length=args.max_input_sequence_length, batch_size=args.batch_size, device=args.device)
            else:
            ...
            for epoch in range(args.num_epochs):
                ...
                for batch_idx, train_data_indices in enumerate(train_idx_data_loader_tqdm):
                    ...
                    if args.model_name in ['TGAT', 'CAWN', 'TCL']:
                        ...
                        batch_src_node_embeddings, batch_dst_node_embeddings = model[0].compute_src_dst_node_temporal_embeddings(src_node_ids=batch_src_node_ids,
                        ...
                        # two Tensors, with shape (batch_size, node_feat_dim)
                        batch_neg_src_node_embeddings, batch_neg_dst_node_embeddings = model[0].compute_src_dst_node_temporal_embeddings(src_node_ids=batch_neg_src_node_ids,
                        ...
                        batch_neg_src_node_embeddings, batch_neg_dst_node_embeddings = model[0].compute_src_dst_node_temporal_embeddings(batch_edge_ids,src_node_ids=batch_neg_src_node_ids,
                                                                              dst_node_ids=batch_neg_dst_node_ids,
                                                                              node_interact_times=batch_node_interact_times, comp_pos_neg='neg')
                        # get temporal embedding of source and destination nodes
                        # two Tensors, with shape (batch_size, node_feat_dim)
                        batch_src_node_embeddings, batch_dst_node_embeddings = model[0].compute_src_dst_node_temporal_embeddings(batch_edge_ids,src_node_ids=batch_src_node_ids,
                                                                              dst_node_ids=batch_dst_node_ids,
                    ...
                    gpu_memory = torch.cuda.memory_reserved() / 1024 ** 2  # MB
                    train_idx_data_loader_tqdm.set_description(
                        f'Epoch: {epoch + 1}, train for the {batch_idx + 1}-th batch, train loss: {loss.item()},GPU Memory: {gpu_memory:.2f} MB')
                ...
                model[0].reset_tppr()
                tppr_filled = True
                model[0].fill_tppr(train_data.src_node_ids, train_data.dst_node_ids, train_data.node_interact_times,
                                               train_data.edge_ids, tppr_filled)
                ...
                train_temp_node_memory = model[0].back_up_memory()
                train_tppr_backup = model[0].backup_tppr()
                ...
                model[0].reload_memory(train_temp_node_memory)

                val_tppr_backup = model[0].backup_tppr()
                model[0].restore_tppr(train_tppr_backup)
