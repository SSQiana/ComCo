...
class GraphMixer(nn.Module):
    ...
    def __init__(self, node_raw_features: np.ndarray, edge_raw_features: np.ndarray, neighbor_sampler: NeighborSampler,
        ...
        self.updated_node_embeddings = torch.zeros(node_raw_features.shape[0], node_raw_features.shape[1]).to(device)
    ...
    def compute_src_dst_node_temporal_embeddings(self, src_node_ids: np.ndarray, dst_node_ids: np.ndarray,
        ...
        n_src_node_embeddings, n_dst_node_embeddings = self.update_node_temporal_embeddings(src_node_embeddings,dst_node_embeddings,src_node_ids, dst_node_ids)
    ...
    def update_node_temporal_embeddings(self, src_node_embeddings, dst_node_embeddings, src_node_ids, dst_node_ids):
        ...
        for i in range(len(src_node_ids)):
            src_emb = self.updated_node_embeddings[src_node_ids[i]].data
            dst_emb = self.updated_node_embeddings[dst_node_ids[i]].data
            src_new = self.update_embeddings(src_node_embeddings[i], src_emb)
            dst_new = self.update_embeddings(dst_node_embeddings[i], dst_emb)

            updated_node_embeddings[src_node_ids[i]] = src_new
            updated_node_embeddings[dst_node_ids[i]] = dst_new

        return updated_node_embeddings[src_node_ids], updated_node_embeddings[dst_node_ids]
    ...
    def compute_node_temporal_embeddings(self, node_ids: np.ndarray, node_interact_times: np.ndarray,
        ...
        neighbor_node_ids, neighbor_edge_ids, neighbor_times = self.neighbor_sampler.get_historical_neighbors(node_ids=node_ids,
